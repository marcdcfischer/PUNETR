...
PYTHON="/opt/pyenv/versions/3.10.10/bin/python"  # EDIT ME
...
batch_size=6
batch_size_ds=6
grad_batches=3  # Aim for batch_size * grad_batches = ~16
grad_batches_ds=3
max_epochs=300
max_epochs_downstream=120
learning_rate=1e-4
learning_rate_Downstream=2e-4
learning_rate_instructions=1e-3
learning_rate_instructions_downstream=2e-3
loss_weight_segmentation=2e-2
loss_weight_sim_protos=1e-2
tokens_per_instruction=24
tokens_per_instruction_seg=24
...
if [ "${15}" == "none" ]; then
  ...
  ckpt="--ckpt_run_name ${15}"
  echo "Using ckpt ${15}."
fi

# ablations
ablations="${*:17}"  # Anything passed (directly) as flag during shell script call.
ablations="${ablations}"
...
parameters="--learning_rate ${learning_rate} --learning_rate_downstream ${learning_rate_downstream} --learning_rate_instructions ${learning_rate_instructions} --learning_rate_instructions_downstream ${learning_rate_instructions_downstream}"
...
if [ "${dataset}" == "tseg" ]; then parameters="${parameters} --out_channels ${out_channels} --domain ${domain}"; fi;
...
if [ "$downstream" == true ]; then parameters="${parameters} --batch_size ${batch_size_ds} --accumulate_grad_batches ${grad_batches_ds}"; else parameters="${parameters} --batch_size ${batch_size} --accumulate_grad_batches ${grad_batches}"; fi;
if [ "${architecture}" == "wip" ] || [ "${architecture}" == "wip_simple" ] || [ "${architecture}" == "wip_recon" ]; then
  parameters="${parameters} --label_indices_base ${label_indices_base} --tokens_per_instruction ${tokens_per_instruction} --tokens_per_instruction_seg ${tokens_per_instruction_seg}"
  parameters="${parameters} --patch_size_students ${patch_size_students} --patch_size_teacher ${patch_size_teacher} --attn_window_size ${attn_window_size} --monai_attn_window_size ${monai_attn_window_size}"
  parameters="${parameters} --prompting_variant ${prompting_variant} --adaptation_variant ${adaptation_variant} --model_variant ${model_variant}"
  parameters="${parameters} ${pretrained} ${monai_ckpts}"
fi;
...
tags="${tags} lr_${learning_rate} lrds_${learning_rate_downstream} lri_${learning_rate_instructions} lrids_${learning_rate_instructions_downstream} bs_${batch_size} bsds_${batch_size_ds} na_${num_annotated}"
if [ "$loss_meta" == true ]; then tags="${tags} loss_meta"; fi;
if [ "$loss_self" == true ]; then tags="${tags} loss_self"; fi;
if [ "${15}" != "none" ]; then tags="${tags} ckpt_${15:17}"; fi;
if [ "$downstream" == true ]; then tags="${tags} downstream"; fi;
if [ "${architecture}" == "wip" ] || [ "${architecture}" == "wip_simple" ] || [ "${architecture}" == "wip_recon" ]; then
  tags="${tags} tk_${tokens_per_instruction} tks_${tokens_per_instruction_seg} pv_${prompting_variant} av_${adaptation_variant} mv_${model_variant} pt_${cfg_pretrained}"
