...
def _gen_mesh_grid(image_size: Tuple[int, int, int]):
    coord_grid = torch.stack(torch.meshgrid(torch.arange(image_size[0]), torch.arange(image_size[1]), torch.arange(image_size[2]), indexing='ij'), dim=0).float()  # [3, I (H), J (W), K (D)] from input shape [3, H, W, D]
