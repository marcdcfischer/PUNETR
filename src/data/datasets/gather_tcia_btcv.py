...
def _gather_data(dir_images: str,
    ...
    if 'btcv' in domains:
        ...
        if pseudo_segmentation:
            paths_masks_pseudo_btcv = sorted((plb.Path(dir_masks) / 'processed_btcv').rglob('label*_pseudo.nii.gz'))
            data_dirs['pseudos'].extend(paths_masks_pseudo_btcv)
    ...
    if debug:
        ...
        viewer = nib.viewers.OrthoSlicer3D(np.array(np.stack([nib.load(paths_images_tcia[0]).get_fdata(),
                                                              nib.load(paths_masks_tcia[0]).get_fdata(),
                                                              nib.load(paths_masks_pseudo_tcia[0]).get_fdata()], axis=-1)))
        viewer.show()
        viewer = nib.viewers.OrthoSlicer3D(np.array(np.stack([nib.load(paths_images_btcv[0]).get_fdata(),
                                                              nib.load(paths_masks_btcv[0]).get_fdata(),
                                                              nib.load(paths_masks_pseudo_btcv[0]).get_fdata()], axis=-1)))
        viewer.show()
...
def add_data_specific_args(parent_parser):
    ...
    parser.add_argument('--dir_images', default='/path/to/my/data/', type=str)  # EDIT ME
    parser.add_argument('--dir_masks', default='/path/to/my/data/', type=str)  # EDIT ME
    ...
    parser.add_argument('--additive_alpha', default=[0., 0.07590447, 0.13365165, 0.85956404, 1.67616135, 0.01332489, 0.05445146, 0.29287293, 0.25115765], type=float, nargs=9)  # Additive alpha value based on foreground / background ratio. 0 for background.
    ...
    parser.add_argument('--normalize_data', default=False, type=bool)
    return parser
