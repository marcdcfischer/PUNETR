...
def fetch_ckpt(hparams, object_key: str = '', bucket_name: Optional[str] = 'my_bucket', download_file_path: str = '', digits=6):
    ...
    if not object_key:
        if not hparams.ckpt_select_last:
            ...
            elif 'dice' in objects[0].key:  # based on val_dice_mean
                score_ = [round(float(x_), hparams.ckpt_decimals) for x_ in score_]  # Round to 2 to prefer last ckpt if results are close.
...
def add_aws_specific_args(parent_parser):
    ...
    parser.add_argument('--ckpt_decimals', type=int, default=2)
