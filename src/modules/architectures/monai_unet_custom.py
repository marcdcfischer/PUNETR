...
from monai.utils import alias, deprecated_arg, export, SkipMode, look_up_option
...
class UNet(nn.Module):
    ...
    def _get_connection_block(self, down_path: nn.Module, up_path: nn.Module, subblock: nn.Module) -> nn.Module:
        ...
        """
        return nn.Sequential(down_path, SkipConnectionCustom(subblock), up_path)
...
class SkipConnectionCustom(nn.Module):
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "cat":
            assert all([x_ - y_ <= 1 for x_, y_ in zip(y.shape[2:], x.shape[2:])])  # Make sure difference in dims is not bigger than 1 (so that it only covers mild padding differences)
            assert self.dim == 1
            return torch.cat((y[..., :x.shape[2], :x.shape[3], :x.shape[4]], x), dim=self.dim)
        if self.mode == "add":
