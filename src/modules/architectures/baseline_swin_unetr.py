...
from src.modules.architectures.monai_swin_unetr_custom import SwinUNETR
import math
...
class MonaiSwinUNETR(nn.Module):
    def __init__(self,
        ...
        self.pad = False  # atm hardcoded

        self.net = SwinUNETR(img_size=self.img_size,
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.pad and any([x_ < y_ for x_, y_ in zip(x.shape[2:], self.img_size)]):
            x_size = x.shape[2:]
            ...
            x = F.pad(x, tuple(reversed(paddings)), mode='constant', value=min(x.min(), 0.))  # F.pad needs reverse order (starting from last)
