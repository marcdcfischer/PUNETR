...
class MomentumModel(nn.Module):
    def __init__(self,
        ...
        # Architecture
        if self.conf.architecture_wip.lower() == 'deep':
        ...
        if self.conf.monai_swin_vit_pretrained:
            if self.conf.ckpt is None and self.conf.ckpt_run_name is None:
                ...
                if self.conf.model_variant != 'swin_unetr':
                    warnings.warn(f'Model variant {self.conf.model_variant} differs from the pre-trained model variant (swin_unetr).')
                weights = torch.load(self.conf.monai_swin_vit_ckpt)
                self.network_student.load_swin_vit(weights=weights)
            else:
                raise ValueError('Trying to load conflicting ckpts.')
    ...
    def forward(self,
                ...
                x_proto: Optional[torch.Tensor] = None,
        ...
        dict_out_teacher, dict_out_proto = None, None
        ...
        if x_proto is not None:
            with torch.no_grad():
                dict_out_proto = self.network_teacher(x_proto, label_indices=label_indices, pseudo_indices_subject=pseudo_indices_subject, pseudo_indices_label=pseudo_indices_label, mode_label=mode_label, mode_loss=mode_loss)
    ...
    def add_model_specific_args(parent_parser):
        ...
        if hparams_tmp.architecture_wip.lower() == 'deep':
