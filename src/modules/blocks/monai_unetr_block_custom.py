...
class UnetrUpBlockCustom(nn.Module):
    ...
    def __init__(
        ...
        if res_block:
            self.conv_block = UnetResBlock(
                spatial_dims,
                out_channels + out_channels if skip else out_channels,
                out_channels,
    ...
    def forward(self,
        ...
        if self.skip:
            assert all([x_ - y_ <= 1 for x_, y_ in zip(skip.shape[2:], out.shape[2:])])  # Make sure difference in dims is not bigger than 1 (so that it only covers mild padding differences)
            out = torch.cat((out[..., :skip.shape[2], :skip.shape[3], :skip.shape[4]], skip), dim=1)
